...
COMMENT
ON COLUMN audit.logged_actions.hasura_user IS 'Hasura user whose statement caused the audited event';
COMMENT
ON COLUMN audit.logged_actions.hasura_role IS 'Hasura role of user whose statement caused the audited event';
COMMENT
ON COLUMN audit.logged_actions.hasura_user_id IS 'Hasura user-id of user whose statement caused the audited event';
COMMENT
...
COMMENT
ON COLUMN audit.logged_actions.row_id IS 'Row _id of the row in the given table of the given schema on which operation was performed.';
COMMENT
...
CREATE INDEX logged_actions_hasura_user_id_idx ON audit.logged_actions (hasura_user_id);
CREATE INDEX logged_actions_action_tstamp_tx_stm_idx ON audit.logged_actions (action_tstamp_stm);

CREATE
OR REPLACE FUNCTION audit.if_modified_func() RETURNS TRIGGER AS $body$
DECLARE
...
= ROW(
        ...
        current_query(),                              -- top-level query or queries from client
    ...
    IF
NOT TG_ARGV[0]::boolean IS DISTINCT FROM 'f'::boolean THEN
        audit_row.client_query = NULL;
END IF;

    IF
TG_ARGV[1] IS NOT NULL THEN
        excluded_cols = TG_ARGV[1]::text[];
END IF;

    IF
(TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
        audit_row.row_id = OLD._id;
        old_r
= to_jsonb(OLD);
        new_r
= to_jsonb(NEW);
...
(TG_OP = 'DELETE' AND TG_LEVEL = 'ROW') THEN
        audit_row.row_id = OLD._id;
        audit_row.row_data
= to_jsonb(OLD) - excluded_cols;
    ELSIF
(TG_OP = 'INSERT' AND TG_LEVEL = 'ROW') THEN
        audit_row.row_id = NEW._id;
        audit_row.row_data
= to_jsonb(NEW) - excluded_cols;
    ELSIF
...
COMMENT
ON FUNCTION audit.if_modified_func() IS $body$
Track changes to a table at the statement and/or row level.

Optional parameters to trigger in
CREATE TRIGGER call:
...
CREATE
OR REPLACE FUNCTION audit.audit_table(target_table regclass, audit_rows boolean, audit_query_text boolean, ignored_cols text[]) RETURNS void AS $body$
DECLARE
...
COMMENT
ON FUNCTION audit.audit_table(regclass, boolean, boolean, text[]) IS $body$
...
Arguments:
   ...
   ignored_cols:     Columns to exclude from
update diffs, ignore updates that change only ignored cols.
    $body$;

-- Pg doesn't allow variadic calls with 0 params, so provide a wrapper
CREATE
OR REPLACE FUNCTION audit.audit_table(target_table regclass, audit_rows boolean, audit_query_text boolean) RETURNS void AS $body$
...
--
CREATE
OR REPLACE FUNCTION audit.audit_table(target_table regclass) RETURNS void AS $body$
SELECT audit.audit_table($1, BOOLEAN 't', BOOLEAN 't');
$body$
LANGUAGE 'sql';

COMMENT
ON FUNCTION audit.audit_table(regclass) IS $body$
Add auditing support to the given table. Row-level changes will be logged with full client query text. No cols are ignored.
$body$;

CREATE
OR REPLACE FUNCTION deaudit_table(target_table regclass) RETURNS void AS $body$
BEGIN
...
COMMENT
ON FUNCTION deaudit_table(regclass) IS $body$
Remove auditing support to the given table.
$body$;

CREATE
OR REPLACE VIEW audit.tableslist AS
...
COMMENT
ON VIEW audit.tableslist IS $body$
